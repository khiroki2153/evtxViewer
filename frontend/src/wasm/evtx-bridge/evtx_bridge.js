/* @ts-self-types="./evtx_bridge.d.ts" */
 ...
 * @returns {string}
 */
export function parse_evtx(data) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.parse_evtx(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('evtx_bridge_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
